                ...
                <div class="row align-items-center">
                    ...
                    <div class="col-lg-4 text-lg-end mt-3 mt-lg-0">
                        <a href="{{ route('content.index') }}" class="btn btn-light btn-lg">
                            <i class="bi bi-plus-circle me-2"></i>{{ __('translation.dashboard.generate_content') }}
                        </a>
                    </div>
                </div>
            </div>
                ...
                <div class="card h-100 border-0 shadow-sm rounded-3">
                    <div class="card-body">
                        <div class="d-flex justify-content-between align-items-start mb-3">
                            ...
                            @if(isset($growthStats['subscriptions']))
                                <span class="badge {{ $growthStats['subscriptions']['growth'] >= 0 ? 'bg-success' : 'bg-danger' }} bg-opacity-10 {{ $growthStats['subscriptions']['growth'] >= 0 ? 'text-success' : 'text-danger' }}">
                                    <i class="bi bi-arrow-{{ $growthStats['subscriptions']['growth'] >= 0 ? 'up' : 'down' }}"></i>
                                    {{ abs($growthStats['subscriptions']['growth']) }}%
                                </span>
                            @endif
                        </div>
                        <h3 class="fw-bold text-success mb-1">{{ number_format($statistics['subscriptions'] ?? 0) }}</h3>
                        <small class="text-muted">{{ __('translation.dashboard.stats.active_subscriptions') }}</small>
                    </div>
                </div>
            </div>
                ...
                <div class="card h-100 border-0 shadow-sm rounded-3">
                    <div class="card-body">
                        <div class="d-flex justify-content-between align-items-start mb-3">
                            ...
                            @if(isset($growthStats['contents']))
                                <span class="badge {{ $growthStats['contents']['growth'] >= 0 ? 'bg-success' : 'bg-danger' }} bg-opacity-10 {{ $growthStats['contents']['growth'] >= 0 ? 'text-success' : 'text-danger' }}">
                                    <i class="bi bi-arrow-{{ $growthStats['contents']['growth'] >= 0 ? 'up' : 'down' }}"></i>
                                    {{ abs($growthStats['contents']['growth']) }}%
                                </span>
                            @endif
                        </div>
                        <h3 class="fw-bold text-info mb-1">{{ number_format($statistics['contents'] ?? 0) }}</h3>
                        <small class="text-muted">{{ __('translation.dashboard.stats.contents_generated') }}</small>
                    </div>
                </div>
            </div>
                    ...
                    <div class="card-header bg-transparent border-0">
                        <div class="d-flex align-items-center justify-content-between">
                            <div class="d-flex align-items-center gap-2">
                                ...
                                <h5 class="card-title fw-semibold mb-0">{{ __('translation.dashboard.top_content_types') }}</h5>
                            </div>
                            <span class="badge bg-primary bg-opacity-10 text-primary">{{ __('translation.dashboard.top_badge') }} 5</span>
                        </div>
                    </div>
                    <div class="card-body">
                        @if(isset($topContentTypes) && $topContentTypes->count() > 0)
                            <div class="d-flex flex-column gap-3">
                                @foreach($topContentTypes as $index => $type)
                                    <div class="d-flex align-items-center gap-3">
                                        ...
                                        <div class="flex-grow-1 min-width-0">
                                            <div class="d-flex align-items-center justify-content-between mb-1">
                                                <span class="fw-semibold text-truncate me-2">{{ $type->name }}</span>
                                                <span class="badge rounded-pill" style="background-color: {{ $type->color ?? '#4A90D9' }};">
                                                    {{ number_format($type->generated_contents_count) }}
                                                </span>
                                            </div>
                                            <div class="progress" style="height: 6px;">
                                                <div class="progress-bar" role="progressbar" 
                                                    style="width: {{ round(($type->generated_contents_count / max($statistics['contents'], 1)) * 100) }}%; background-color: {{ $type->color ?? '#4A90D9' }};"
                                                    aria-valuenow="{{ round(($type->generated_contents_count / max($statistics['contents'], 1)) * 100) }}" 
                                                    aria-valuemin="0" aria-valuemax="100">
                                                </div>
                                            </div>
                                            <small class="text-muted">{{ round(($type->generated_contents_count / max($statistics['contents'], 1)) * 100) }}% {{ __('translation.dashboard.of_total_content') }}</small>
                                        </div>
                                    </div>
                                @endforeach
                            </div>
                                        ...
                                        <div class="flex-grow-1">
                                            <div class="d-flex align-items-center justify-content-between">
                                                <div>
                                                    ...
                                                    <small class="text-muted">{{ number_format($specialty->generated_contents_count) }} {{ __('translation.dashboard.contents') }}</small>
                                                </div>
                                                @if($index === 0)
                ...
                <div class="card border-0 shadow-sm rounded-3">
                    <div class="card-header bg-transparent border-0 d-flex justify-content-between align-items-center">
                        <div class="d-flex align-items-center gap-2">
                            ...
                            <h5 class="card-title fw-semibold mb-0">{{ __('translation.dashboard.recent_contents') }}</h5>
                        </div>
                        <a href="{{ route('content.history') }}" class="btn btn-sm btn-outline-primary rounded-pill">
                    ...
                    <div class="card-body p-0">
                            ...
                            <div class="table-responsive">
                                <table class="table table-hover align-middle mb-0">
                                    <thead class="bg-light">
                                        <tr>
                                            <th class="border-0 text-muted small fw-semibold px-4 py-3">{{ __('translation.dashboard.table.user') }}</th>
                                            <th class="border-0 text-muted small fw-semibold">{{ __('translation.dashboard.table.content_type') }}</th>
                                            <th class="border-0 text-muted small fw-semibold">{{ __('translation.dashboard.table.specialty') }}</th>
                                            ...
                                            <tr>
                                                <td class="px-4">
                                                    <div class="d-flex align-items-center gap-2">
                                                        ...
                                                        <div>
                                                            <div class="fw-medium small">{{ $content->user->name ?? 'N/A' }}</div>
                                                            <small class="text-muted" style="font-size: 0.75rem;">{{ Str::limit($content->user->email ?? '', 20) }}</small>
                                                        </div>
                                                    </div>
                                                </td>
                                                <td>
                                                    @if($content->contentType)
                                                        <span class="badge rounded-pill" style="background-color: {{ $content->contentType->color ?? '#4A90D9' }};">
                                                ...
                                                <td>
                                                    <span class="small text-muted">{{ $content->specialty->name ?? '-' }}</span>
                                                </td>
                                                <td>
                        ...
                        @else
                            <div class="text-center py-5">
                                ...
                                <p class="text-muted mt-3">{{ __('translation.dashboard.no_recent_contents') }}</p>
                            </div>
                        @endif
                    </div>
                </div>
                            ...
                            <div class="col-6">
                                <a href="{{ route('content.index') }}" class="text-decoration-none">
                                    <div class="text-center p-3 rounded-3 bg-primary bg-opacity-10 hover-lift" style="transition: all 0.3s;">
                                        ...
                                        <div class="fw-semibold small text-primary">{{ __('translation.dashboard.new_content') }}</div>
                                    </div>
                                </a>
                            </div>
                            <div class="col-6">
