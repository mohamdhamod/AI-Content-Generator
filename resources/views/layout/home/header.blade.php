		...
		<div class="collapse navbar-collapse {{ $isRtl ? 'text-end' : '' }} mt-3 mt-lg-0 pt-2 pt-lg-0" id="homeNavbar">
			<div class="d-flex flex-column flex-lg-row align-items-stretch align-items-lg-center gap-3 w-100">
				<ul class="navbar-nav {{ $navSpacerClass }} mb-0">
					...
					@auth
                        @canany([\App\Enums\PermissionEnum::CONTENT_GENERATION_CREATE,])
                            <li class="nav-item"><a href="{{ route('content.index') }}" class="nav-link">{{ __('translation.sidebar.generate_content') ?? 'Generate Content' }}</a></li>
                            <li class="nav-item"><a href="{{ route('content.favorites') }}" class="nav-link"><i class="bi bi-star me-1"></i>{{ __('translation.sidebar.favorites') ?? 'Favorites' }}</a></li>
                        @endcanany
					@endauth
