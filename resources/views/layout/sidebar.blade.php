...
<div class="sidenav-menu" id="sidenavMenu">
    <div class="scrollbar" style="height: calc(100% - 41px); overflow-y: auto;">
        ...
        <ul class="side-nav">
            @canany([
                \App\Enums\PermissionEnum::CONTENT_GENERATION_CREATE,
                \App\Enums\PermissionEnum::CONTENT_GENERATION_VIEW,
            ])
                <li class="side-nav-title mt-3">{{ __('translation.sidebar.content_generation') ?? 'Content Generation' }}</li>
                <li class="side-nav-item">
                    <a href="{{ route('content.index') }}" class="side-nav-link {{ request()->routeIs('content.*') ? 'active' : '' }}">
                        <i class="menu-icon bi bi-magic"></i>
                        <span class="menu-text">{{ __('translation.sidebar.generate_content') ?? 'Generate Content' }}</span>
                    </a>
                </li>
            @endcanany

            @canany([
                \App\Enums\PermissionEnum::MANAGE_SUBSCRIPTIONS,
            ])
                <li class="side-nav-title mt-3">{{ __('translation.sidebar.subscriptions') }}</li>

                <li class="side-nav-item">
                    <button class="side-nav-link" type="button" data-bs-toggle="collapse" data-bs-target="#subscriptionsMenu" aria-expanded="false">
                    ...
                    <div class="collapse" id="subscriptionsMenu">
                        <ul class="sub-menu">
                            <li class="side-nav-item">
                                <a href="{{ route('subscriptions.dashboard.index') }}" class="side-nav-link {{ request()->routeIs('subscriptions.dashboard.index') ? 'active' : '' }}">
                                    <i class="menu-icon bi bi-check2-circle"></i>
                                    <span class="menu-text">{{ __('translation.sidebar.active_subscriptions') }}</span>
                                </a>
                            </li>
                            <li class="side-nav-item">
                                <a href="{{ route('subscriptions.dashboard.manage.index') }}" class="side-nav-link {{ request()->routeIs('subscriptions.dashboard.manage.index') ? 'active' : '' }}">
                                    <i class="menu-icon bi bi-gear"></i>
                                    <span class="menu-text">{{ __('translation.sidebar.manage_subscriptions') }}</span>
                                </a>
                            </li>
                        </ul>
                    </div>
