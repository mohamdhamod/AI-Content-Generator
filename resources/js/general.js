/**
 * ============================================================================
 * AI Medical Content Generator - General JavaScript Utilities
 * ============================================================================
 * 
 ...
 * 
 * Features:
 * - Content Management (copy, download, favorite)
...
const FormManager = {
    ...
    async handleFormSubmit(event, form) {
        ...
        if (!submitButton) {
            console.warn('Submit button not found in form or modal');
        }
            ...
            if (contentType.includes('application/json')) {
                ...
                if (response.ok && !serverIndicatesFailure) {
                    ...
                    if (result.redirect) {
                        ...
                        // Don't restore button - let the redirect happen immediately
                    ...
                    if (result && result.redirect) {
                        ...
                        // Don't restore button - redirect immediately
                        window.location.href = result.redirect;
                        return;
                    }
...
if (!window.handleSubmit) {
    window.handleSubmit = function(selector, options = {}) {
        ...
        form.addEventListener('submit', function(event) {
            ...
            try {
                FormManager.handleFormSubmit(event, form);
            } catch (e) {
...
if (!window.__recaptchaGlobalSubmitListenerAdded) {
    ...
    document.addEventListener('submit', async function (e) {
        ...
        try {
            await FormManager.handleFormSubmit(e, form);
        } catch (err) {
...
// ============================================================================
// PHASE 4: ADVANCED FEATURES - CONTENT MANAGEMENT UTILITIES
// ============================================================================

/**
 * Content Management Object
 * Handles content operations like copy, download, favorite
 */
window.ContentManager = {
    /**
     * Copy content to clipboard with SweetAlert notification
     * @param {string} contentId - The content element ID
     * @param {string} successMessage - Success message
     */
    copyContent: function(contentId = 'content-output', successMessage = 'Content copied successfully!') {
        ...
        navigator.clipboard.writeText(content).then(function() {
            SwalHelper.toast(successMessage, 'success', { background: '#d1e7dd', iconColor: '#0f5132' });
        }).catch(function(err) {
            console.error('Copy failed:', err);
            SwalHelper.toast(window.i18n?.messages?.copy_failed || 'Copy failed', 'error');
        });
    },
     ...
     */
    downloadContent: function(contentId = 'content-output', filename = 'generated-content.txt') {
        const element = document.getElementById(contentId);
        if (!element) {
            console.error(`Element with ID "${contentId}" not found`);
            return;
     ...
     * @param {string} toggleUrl - The API endpoint URL
     * @param {object} messages - Success/error messages
     */
    toggleFavorite: function(button, toggleUrl, messages = {}) {
        ...
        icon.className = 'bi bi-heart-pulse me-1';
        ...
        .then(data => {
            if (data.success) {
                ...
                button.dataset.isFavorited = newIsFavorited;
                
                icon.style.transform = 'scale(1.5)';
                setTimeout(() => {
                    icon.className = newIsFavorited ? 'bi bi-heart-fill me-1' : 'bi bi-heart me-1';
                    text.textContent = newIsFavorited ? 
                        (messages.unfavorite || window.i18n?.messages?.unfavorite || 'Remove from Favorites') : 
                        (messages.favorite || window.i18n?.messages?.favorite || 'Add to Favorites');
                    icon.style.transform = 'scale(1)';
                }, 200);
                ...
                throw new Error(data.message || window.i18n?.messages?.failed_to_toggle_favorite || 'Failed to toggle favorite');
            }
        })
        .catch(error => {
            ...
            SwalHelper.toast(messages.error || window.i18n?.messages?.failed_to_update_favorite_status || 'Failed to update favorite status', 'error', { timer: 3000 });
        })
        .finally(() => {
...
window.SeoManager = {
     ...
     * @param {function} callback - Callback function to display results
     */
    analyzeSeo: function(url, focusKeyword, metaDescription, callback) {
        SwalHelper.showLoading(
            window.i18n?.messages?.analyzing_seo || 'Analyzing SEO...',
            window.i18n?.messages?.analyzing_seo_text || 'Please wait while we analyze your content.'
        );
        ...
        .then(data => {
            ...
            if (data.success) {
                ...
                SwalHelper.error(
                    window.i18n?.messages?.seo_analysis_failed || 'SEO Analysis Failed',
                    data.message || window.i18n?.messages?.failed_to_analyze_seo || 'Failed to analyze SEO'
                );
            }
        })
        .catch(error => {
            console.error('Error:', error);
            SwalHelper.error(
                window.i18n?.messages?.error || 'Error',
                window.i18n?.messages?.network_error || 'Network error occurred'
            );
        });
    },
    ...
    getGradeClass: function(grade) {
        ...
        return gradeMap[grade] || 'text-secondary';
    },

    displayResults: function(data, containerId = 'seoResults') {
        ...
        if (overallScoreEl) {
            overallScoreEl.textContent = Math.round(data.overall_score);
            overallScoreEl.className = `display-4 fw-bold text-${this.getScoreColor(data.overall_score)}`;
        }

        const gradeEl = document.getElementById('seoGrade');
        if (gradeEl) {
            gradeEl.textContent = data.grade;
            gradeEl.className = `badge bg-${this.getScoreColor(data.overall_score)} fs-4`;
        }
        ...
        Object.keys(breakdown).forEach(category => {
            ...
            if (progressBar) {
                ...
                progressBar.className = `progress-bar bg-${this.getScoreColor(score)}`;
    ...
    resetAnalysis: function(containerId = 'seoResults') {
        const container = document.getElementById(containerId);
        if (container) {
...
window.SocialManager = {
    ...
    loadPreview: function(platform, url) {
        ...
        previewContent.innerHTML = `
            <div class="text-center py-5">
                ...
                <p class="mt-3">Generating ${platform} preview...</p>
            </div>
        `;
        ...
        .then(data => {
            if (data.success) {
                this.currentPreviewData = data.preview;
                this.renderPreview(data.preview);
            } else {
                throw new Error(data.message || window.i18n?.messages?.failed_to_load_preview || 'Failed to load preview');
            }
        })
        .catch(error => {
    ...
    renderPreview: function(preview) {
        ...
        if (preview.platform === 'twitter') {
            html = this.renderTwitterPreview(preview);
        } else if (preview.platform === 'facebook') {
            html = this.renderFacebookPreview(preview);
        } else if (preview.platform === 'linkedin') {
            html = this.renderLinkedInPreview(preview);
        } else if (preview.platform === 'instagram') {
            html = this.renderInstagramPreview(preview);
        }

        previewContent.innerHTML = html;
    },

    renderTwitterPreview: function(preview) {
        return `
            <div class="card border-0 shadow-sm" style="max-width: 500px; margin: 0 auto;">
                <div class="card-body">
                    ...
                    <div class="mb-3">${preview.text || ''}</div>
    ...
    renderFacebookPreview: function(preview) {
        return `
            <div class="card border-0 shadow-sm" style="max-width: 500px; margin: 0 auto;">
                <div class="card-body">
                    <div class="d-flex align-items-center mb-3">
    ...
    renderLinkedInPreview: function(preview) {
        return `
    ...
    renderInstagramPreview: function(preview) {
        return `
            <div class="card border-0 shadow-sm" style="max-width: 400px; margin: 0 auto;">
                ...
                <div class="card-body p-0">
                    ...
                    <div class="p-3">
                        ...
                        <div class="small">
                            <span class="fw-bold">your_username</span> ${preview.text || ''}
                        </div>
                    </div>
                </div>
            </div>
    ...
    copySocialPreview: function(platform) {
        const textToCopy = this.currentPreviewData.text || '';
        
        if (!textToCopy) {
            SwalHelper.toast(window.i18n?.messages?.no_content_to_copy || 'No content to copy', 'warning');
            return;
        }

        navigator.clipboard.writeText(textToCopy).then(function() {
            SwalHelper.toast(`${platform} content copied!`, 'success');
        }).catch(function(err) {
            console.error('Copy failed:', err);
        });
    }
...
window.VersionManager = {
    loadHistory: function(url, callback) {
        fetch(url, {
            headers: {
                'Accept': 'application/json',
                'X-CSRF-TOKEN': Utils.getCSRFToken()
    ...
    restoreVersion: function(versionId, url) {
        SwalHelper.confirm(
            window.i18n?.messages?.restore_version_question || 'Restore Version?',
            window.i18n?.messages?.restore_version_text || 'This will replace the current content with the selected version.',
            {
                confirmButtonText: window.i18n?.messages?.yes_restore_it || 'Yes, restore it!',
                cancelButtonText: window.i18n?.messages?.cancel || 'Cancel'
            }
        ).then((result) => {
            if (result.isConfirmed) {
                this.performRestore(versionId, url);
            }
        });
    },

    performRestore: function(versionId, url) {
        SwalHelper.showLoading(
            window.i18n?.messages?.restoring || 'Restoring...',
            window.i18n?.messages?.restoring_text || 'Please wait while we restore the version.'
        );
        ...
        .then(data => {
            if (data.success) {
                SwalHelper.success(
                    window.i18n?.messages?.version_restored_title || 'Version Restored!',
                    window.i18n?.messages?.version_restored_text || 'The content has been restored successfully.',
                    { timer: 2000 }
                ).then(() => {
                    window.location.reload();
                });
            } else {
                throw new Error(data.message || window.i18n?.messages?.version_restore_failed || 'Failed to restore version');
            }
        })
        .catch(error => {
            console.error('Error:', error);
            SwalHelper.error(
                window.i18n?.messages?.restore_failed || 'Restore Failed',
                error.message || window.i18n?.messages?.version_restore_failed || 'Failed to restore version. Please try again.'
            );
        });
    }
};
...
window.CalendarManager = {
    scheduleContent: function(url, data, callback) {
        SwalHelper.showLoading(
            window.i18n?.messages?.scheduling || 'Scheduling...',
            window.i18n?.messages?.scheduling_text || 'Please wait while we schedule your content.'
        );
        ...
        .then(data => {
            if (data.success) {
                SwalHelper.success(
                    window.i18n?.messages?.content_scheduled || 'Content Scheduled!',
                    data.message || window.i18n?.messages?.content_scheduled_successfully || 'Your content has been scheduled successfully.',
                    { timer: 2500 }
                ).then(() => {
                    if (callback) callback(data);
                    else window.location.reload();
                ...
                throw new Error(data.message || window.i18n?.messages?.failed_to_schedule_content || 'Failed to schedule content');
            }
        })
        .catch(error => {
            console.error('Error:', error);
            SwalHelper.error(
                window.i18n?.messages?.scheduling_failed || 'Scheduling Failed',
                error.message || window.i18n?.messages?.failed_to_schedule_content_text || 'Failed to schedule content. Please try again.'
            );
        });
    },

    publishNow: function(url, callback) {
        SwalHelper.confirm(
            window.i18n?.messages?.publish_now || 'Publish Now?',
            window.i18n?.messages?.publish_now_text || 'This will immediately publish your content.',
            {
                icon: 'question',
                confirmButtonText: window.i18n?.messages?.yes_publish_now || 'Yes, publish now!',
                cancelButtonColor: '#6c757d',
                cancelButtonText: window.i18n?.messages?.cancel || 'Cancel'
            }
        ).then((result) => {
            if (result.isConfirmed) {
                this.performPublish(url, callback);
            }
        });
    },

    performPublish: function(url, callback) {
        SwalHelper.showLoading(
            window.i18n?.messages?.publishing || 'Publishing...',
            window.i18n?.messages?.publishing_text || 'Please wait while we publish your content.'
        );
        ...
        .then(data => {
            if (data.success) {
                SwalHelper.success(
                    window.i18n?.messages?.content_published || 'Content Published!',
                    window.i18n?.messages?.content_published_text || 'Your content is now live.',
                    { timer: 2500 }
                ).then(() => {
                    if (callback) callback(data);
                    else window.location.reload();
                ...
                throw new Error(data.message || window.i18n?.messages?.failed_to_publish_content || 'Failed to publish content');
            }
        })
        .catch(error => {
            console.error('Error:', error);
            SwalHelper.error(
                window.i18n?.messages?.publishing_failed || 'Publishing Failed',
                error.message || window.i18n?.messages?.failed_to_publish_content_text || 'Failed to publish content. Please try again.'
            );
        });
    }
};
...
// ============================================================================
// CHAT MANAGER - Centralized Chat Interface Functions
// ============================================================================
// Handles chat message display, typing indicators, and message utilities
// ============================================================================
...
window.ChatManager = {
    ...
    /**
     * Initialize chat manager with custom container IDs
     */
    init: function(options = {}) {
        this.messagesContainerId = options.messagesContainerId || 'messagesContainer';
        this.messagesAreaId = options.messagesAreaId || 'messagesArea';
        return this;
    },
     ...
     * @param {Object} options - Additional options (avatarIcon, container)
     */
    addMessage: function(content, type, isError = false, options = {}) {
        const containerId = options.containerId || this.messagesContainerId;
        const areaId = options.areaId || this.messagesAreaId;
        const container = document.getElementById(containerId);
        const area = document.getElementById(areaId);
        
        if (!container) {
            console.error('ChatManager: Messages container not found');
            return null;
        }
        ...
        avatar.innerHTML = type === 'assistant' 
            ? (options.assistantIcon || '<i class="fas fa-robot"></i>') 
            : (options.userIcon || '<i class="fas fa-user"></i>');
     ...
     * @returns {string} - Typing indicator ID for later removal
     */
    showTyping: function(options = {}) {
        const containerId = options.containerId || this.messagesContainerId;
        const areaId = options.areaId || this.messagesAreaId;
        const container = document.getElementById(containerId);
        const area = document.getElementById(areaId);
    ...
    /**
     * Create option tag for selected filters
     * @param {string} text - Tag text
     * @param {string} type - Tag type (specialty, topic, contentType)
     * @param {Function} onRemove - Callback when tag is removed
     */
    createOptionTag: function(text, type, onRemove) {
    ...
    /**
     * Send chat message via fetch
     * @param {string} url - API endpoint
     * @param {FormData|Object} data - Message data
     * @param {Object} callbacks - Success/error callbacks
     */
    sendMessage: async function(url, data, callbacks = {}) {
        ...
        try {
            ...
            if (callbacks.onNetworkError) callbacks.onNetworkError(error);
            throw error;
        }
    },
     ...
     */
    autoResizeTextarea: function(textarea, maxHeight = 120) {
        if (!textarea) return;
...
document.addEventListener('DOMContentLoaded', function() {
    console.log('✅ AI Medical Content Generator - JavaScript Utilities Loaded (v4.5)');
    console.log('📦 Available Managers:', {
        ContentManager: typeof window.ContentManager !== 'undefined',
        SeoManager: typeof window.SeoManager !== 'undefined',
        SocialManager: typeof window.SocialManager !== 'undefined',
        RefinementManager: typeof window.RefinementManager !== 'undefined',
        VersionManager: typeof window.VersionManager !== 'undefined',
        CalendarManager: typeof window.CalendarManager !== 'undefined',
        ChatManager: typeof window.ChatManager !== 'undefined',
